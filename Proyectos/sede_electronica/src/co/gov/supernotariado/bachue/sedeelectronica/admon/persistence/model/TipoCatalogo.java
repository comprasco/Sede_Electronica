 ...
 * datos.<br>
 * Esta clase representa la entidad <b>Tipo catalogo</b>, <br>
 * que permite la persistencia en la tabla <b>SDB_PGN_TIPO_CATALOGO</b> de la
 * base de datos.
 * 
 * <br>
 ...
 */
@Entity
@Table(name = "SDB_PGN_TIPO_CATALOGO")
@NamedQueries( value = {
		@NamedQuery(name = "TipoCatalogo.findAll", query = "SELECT s FROM TipoCatalogo s"),
		@NamedQuery(name = "TipoCatalogo.findByCodigo", query = "SELECT s FROM TipoCatalogo s WHERE s.codigo = :codigo")
})
public class TipoCatalogo implements Serializable {
	 ...
	 */
	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "SDB_PGN_TIPO_CATALOGO")
	@SequenceGenerator(sequenceName = "SEC_PGN_TIPO_CATALOGO", allocationSize = 1, name = "SDB_PGN_TIPO_CATALOGO")
	private String id;
	...
	/**
	 * Atributo para definir la propiedad es negocio.
	 */
	@Column(name = "ES_NEGOCIO")
	private String esNegocio;
	...
	/**
	 * Atributo para definir la propiedad sdb pgn catalogos.
	 */
	// bi-directional many-to-one association to SdbPgnCatalogo
	@OneToMany(mappedBy = "tipoCatalogo")
	private List<Catalogo> sdbPgnCatalogos;

	/**
	 * Construye una nueva instancia/objeto de la clase TipoCatalogo.
	 */
	public TipoCatalogo() {
	...
	/**
	 * Obtiene el valor para la propiedad sdb pgn catalogos.
	 *
	 * @return El valor de la propiedad sdb pgn catalogos
	 */
	public List<Catalogo> getSdbPgnCatalogos() {
		return this.sdbPgnCatalogos;
	}

	/**
	 * Cambia/actualiza el valor para la propiedad sdb pgn catalogos.
	 *
	 * @param sdbPgnCatalogos el nuevo valor para la propiedad sdb pgn catalogos
	 */
	public void setSdbPgnCatalogos(List<Catalogo> sdbPgnCatalogos) {
		this.sdbPgnCatalogos = sdbPgnCatalogos;
	}
	 ...
	 */
	public void setEsNegocio(String as_esNegocio) {
		this.esNegocio = as_esNegocio;
	}

	/**
	 * Agrega el/la sdb pgn catalogo.
	 *
	 * @param catalogo el parametro catalogo
	 * @return Resultado para Agrega el/la sdb pgn catalogo retornado como Catalogo
	 */
	public Catalogo addSdbPgnCatalogo(Catalogo catalogo) {
		getSdbPgnCatalogos().add(catalogo);
	...
	/**
	 * Remueve el/la sdb pgn catalogo.
	 *
	 * @param sdbPgnCatalogo el parametro sdb pgn catalogo
	 * @return Resultado para Remueve el/la sdb pgn catalogo retornado como Catalogo
	 */
	public Catalogo removeSdbPgnCatalogo(Catalogo sdbPgnCatalogo) {
		getSdbPgnCatalogos().remove(sdbPgnCatalogo);
		sdbPgnCatalogo.setTipoCatalogo(null);

		return sdbPgnCatalogo;
	}
