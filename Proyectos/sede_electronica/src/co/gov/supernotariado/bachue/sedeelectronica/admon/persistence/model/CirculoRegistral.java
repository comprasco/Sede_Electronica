 ...
 * datos.<br>
 * Esta clase representa la entidad <b>Circulo registral</b>, <br>
 * que permite la persistencia en la tabla <b>SDB_PGN_CIRCULO_REGISTRAL</b> de
 * la base de datos.
 * 
 * <br>
 ...
 */
@Entity
@Table(name = "SDB_PGN_CIRCULO_REGISTRAL")
@NamedQueries( value = {
	@NamedQuery(name = "CirculoRegistral.findAll", query = "SELECT s FROM CirculoRegistral s ORDER BY s.nombre"),
	@NamedQuery(name = "CirculoRegistral.findByCodigo", query = "SELECT s FROM CirculoRegistral s where s.id = :id ")	
})

public class CirculoRegistral implements Serializable {
	 ...
	 */
	@Id
	@SequenceGenerator(name = "SDB_PGN_CIRCULO_REGISTRAL_ID_GENERATOR", sequenceName = "SECUENCES")
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "SDB_PGN_CIRCULO_REGISTRAL_ID_GENERATOR")
	private String id;
	...
	/**
	 * Construye una nueva instancia/objeto de la clase CirculoRegistral.
	 */
